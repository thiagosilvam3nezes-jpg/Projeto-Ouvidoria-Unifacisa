    ...
    if opcao == 1:
        ...
        if len(ouvidorias) == 0:
            ...
            for i in range(len(ouvidorias)):
                print(f"{i+1} - {ouvidorias[i]}")
                
            indice = input("Digite o número da reclamação que deseja atualizar: ")
            
            if indice > 0 and indice < len(ouvidorias):
                nova_reclamacao = input("Digite a nova descrição da reclamação: ")
                ouvidorias[indice] = nova_reclamacao
